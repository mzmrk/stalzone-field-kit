...
test("exhaustively ranks and loads a four-slot weighted build", async ({ page }) => {
  ...
  await expect(movementRow.locator(".objective-share")).toHaveText("33.3%");
  await movementRow.getByRole("button", { name: /Important/ }).click();
  await expect(movementRow.locator(".objective-share")).toHaveText("50%");
  await expect(runningRow.locator(".objective-share")).toHaveText("25%");
  await expect(staminaRow.locator(".objective-share")).toHaveText("25%");
  await page.getByLabel("Optimize Running speed").uncheck();
  await expect(movementRow.locator(".objective-share")).toHaveText("66.7%");
  await expect(staminaRow.locator(".objective-share")).toHaveText("33.3%");
  ...
  await expect(searchButton).toBeDisabled();
  await page.getByLabel("Temperature custom maximum").fill("0.5");
